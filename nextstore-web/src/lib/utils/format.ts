...
export function formatPrice(rials: number, locale: Locale): string {
  ...
  /*
   * ⚠️ مبالغ کوچک با اعشار، بزرگ‌ها بدون آن.
   *
   *    گِردکردن همیشگی به عدد صحیح تا وقتی فقط قیمت محصول (میلیون‌ها
   *    ریال) نمایش داده می‌شد بی‌ضرر بود. اما با آمدن هزینه‌ی ارسال
   *    به متن صفحات، «۵۰ هزار تومان» به **«$1»** تبدیل شد — عددی که
   *    هم غلط به نظر می‌رسد و هم بی‌معنا.
   *
   *    مرز روی ۱۰ دلار است: بالای آن، اعشار فقط شلوغی می‌آورد
   *    («$148.33» به‌جای «$148»)؛ زیر آن، بدون اعشار عدد از دست
   *    می‌رود.
   */
 ...
 *
 * ⚠️ برای نمایش لازم نیست — برای **پروتکل‌ها** لازم است.
 *
 *    شماره‌ی «۰۲۱-۱۲۳۴۵۶۷۸» را که در `tel:` بگذاری، اندروید و iOS
 *    هیچ رقمی در آن نمی‌بینند و لینک بی‌اثر می‌شود؛ نه خطایی، نه
 *    تماسی. همین برای `href` هر جای دیگری که عدد فارسی می‌گیرد صادق
 *    است.
 *
 *    متن قابل مشاهده باید دست‌نخورده بماند: خروجی این تابع فقط در
 *    مقصد لینک می‌نشیند، نه در چیزی که کاربر می‌خواند.
 */
export function toAsciiDigits(value: string): string {
  /* ۰ فارسی در U+06F0 و ٠ عربی در U+0660 شروع می‌شوند */
